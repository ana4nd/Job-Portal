          ...
          <tbody>
            {
              jobs.map((job, index)=>(
                <tr key={index} className='text-gray-700'>
                  ...
                  <td className='py-2 px-4 border-b text-center max-sm:hidden'>{moment(job.date).format('ll')}</td>
                  <td className='py-2 px-4 border-b text-center max-sm:hidden'>{job.location}</td>
                  <td className='py-2 px-4 border-b text-center'>{job.applicants}</td>
                  <td className='py-2 px-4 border-b text-center'>
                    <input onChange={()=>changeJobVisibility(job._id)} className='scale-125 ml-4' type="checkbox" checked={jobs.visible} />
                  </td>
                </tr>
              ))
            }
