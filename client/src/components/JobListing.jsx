...
const JobListing = () => {
  ...
  const handleLocationChange = (location)=>{
    setSelectedLocation(
      prev => prev.includes(location) ? prev.filter((l)=> l != location)
      : [...prev,location]
    )
  }
  ...
  useEffect(()=>{
    ...
    const matchedLocation = job => selectedLocations.length === 0 || selectedLocations.includes(job.location);
    ...
    const newFilterJobs = jobs.slice().reverse().filter(
      job => matchedCategory(job) && matchedLocation(job) && matchedTitle(job) && matchedSearchLocation(job)
    )
